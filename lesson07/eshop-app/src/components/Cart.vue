    ...
    <div class="cart-list">
      <div v-for="item in cart" :key="item.product_id" class="cart-item">
        ...
        <div class="cart-buttons">
          <button @click="clickBtn(item, 'plus')">+</button>
          <button @click="clickBtn(item, 'minus')">-</button>
          <button @click="clickBtn(item, 'del')">Удалить</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    clickBtn(item, action) {
      this.$emit('callChangeCart', item, action);
    }
  }
}
</script>
